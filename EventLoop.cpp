...
int createEventfd()
{
    int evtfd = ::eventfd(0, EFD_NONBLOCK | EFD_CLOEXEC);  // 创建新的eventfd
    if (evtfd < 0)
    {
...
EventLoop::EventLoop()
        ...
        , threadId_(CurrentThread::tid())  // 创建EventLoop时候，获取当前进程的id
...
EventLoop::~EventLoop()
{
    wakeupChannel_->disableAll(); // 取消EventLoop对该Channel事件的关心
...
/**
 * 开启事件循环
 * poll返回活跃的channel  ->   执行对应的事件  -> 事件执行完毕后，执行上层下来的回调doPendingFunctors
 * */
void EventLoop::loop()
{
    looping_ = true;
    ...
    while (!quit_)
    {
        ...
        for (Channel *channel : activeChannels_)
        {
            ...
            // 产生readCallback_时：
            // Channel::handleEvent->Channel::handleEventWithGuard->readCallback_(receiveTime)->Acceptor::handleRead
...
void EventLoop::quit()
{
    quit_ = true;  // 置为true，就会退出loop的循环

    if (!isInLoopThread())   // EventLoop不在自己的线程里，
    {
        wakeup();  // 唤醒阻塞，使得顺利进行到下一次loop
    }
}
...
{
    if (isInLoopThread()) // 当前EventLoop中执行回调,单reactor
    {
        cb();
    }
    else // 在非当前EventLoop线程中执行cb，就需要唤醒EventLoop所在线程执行cb
    {
        queueInLoop(cb);// 唤醒并执行cb
    }
}
...
{
    {  // 指定临界区
        std::unique_lock<std::mutex> lock(mutex_);  // 临界区加锁
     ...
     **/
    // 若此时新加了回调
    // 新加的5个回调加进来了，此时也正在执行dopendingfunctors
    // 那么就唤醒线程，解除阻塞，写入唤醒的8个字节，再写入新加的5个线程
...
{
    uint64_t one = 1;  // 一个64位，8字节
    ssize_t n = read(wakeupFd_, &one, sizeof(one));  // 读8字节
    if (n != sizeof(one))
    {
...
{
    uint64_t one = 1;
    ssize_t n = write(wakeupFd_, &one, sizeof(one));   // 去写8字节来唤醒
    if (n != sizeof(one))
    {
...
{
    poller_->updateChannel(channel);   // channel和eventloop打交道，再间接和poller打交道
}
...
void EventLoop::doPendingFunctors()
{
    ...
    // swap直接换出，减少了锁的临界区范围
    /**
     * 如果不用swap，而是先加锁，一直等到pendingFunctors_执行完
     * 如果我我们在执行过程中，某个cb是queueInLoop，而queueInLoop本身是加锁的
     * 那么会产生死锁现象
     * */

    // 在取出的回调函数执行过程中，又来了5个回调加在pendingFunctor_ 中，那么此时是没法立即执行的
    // 因为在loop中执行完doPendingFunctors后，又回到了阻塞状态EPOLL——WAIT
    //   那么此时也可以用wake_up唤醒
